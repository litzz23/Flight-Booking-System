...
const initDB = async () => {
  try {
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_flights_origin ON flights(origin)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_flights_destination ON flights(destination)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_flights_departure ON flights(departure_time)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_bookings_user ON bookings(user_id)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_bookings_flight ON bookings(flight_id)`)
    ...
    await pool.query(`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS gender VARCHAR(10)`)
    await pool.query(`UPDATE tickets SET gender = 'male' WHERE gender IS NULL OR gender NOT IN ('male', 'female')`)
    await pool.query(`ALTER TABLE tickets ALTER COLUMN gender SET DEFAULT 'male'`)
    await pool.query(`
      DO $$ BEGIN
    ...
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_seats_flight_id ON seats(flight_id)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_seats_seat_number ON seats(seat_number)`)
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_seats_reserved_until ON seats(reserved_until)`)
    ...
    `)
    await pool.query(`ALTER TABLE seat_swap_requests ADD COLUMN IF NOT EXISTS requester_gender VARCHAR(10)`)
    await pool.query(`ALTER TABLE seat_swap_requests ADD COLUMN IF NOT EXISTS target_gender VARCHAR(10)`)
    await pool.query(`
    ...
    await pool.query(`
      ...
      END $$
    `)
    await pool.query(`ALTER TABLE seat_swap_requests ALTER COLUMN requester_gender SET DEFAULT 'male'`)
    await pool.query(`ALTER TABLE seat_swap_requests ALTER COLUMN target_gender SET DEFAULT 'male'`)
    await pool.query(`ALTER TABLE seat_swap_requests ALTER COLUMN requester_gender SET NOT NULL`)
    await pool.query(`ALTER TABLE seat_swap_requests ALTER COLUMN target_gender SET NOT NULL`)
    await pool.query(`
...
${seedValues}
      `)
      console.log('Seed data inserted (~10 flights per KTM destination + return legs)')
    }

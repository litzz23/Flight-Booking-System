...
router.post('/khalti/initiate', authenticate, async (req, res) => {
  ...
  const orderId = (req.body.orderId && String(req.body.orderId).trim()) || crypto.randomUUID()
  const redirectAfterSuccessRaw = req.body.redirectAfterSuccess
  const redirectAfterSuccess =
    typeof redirectAfterSuccessRaw === 'string' && redirectAfterSuccessRaw.startsWith('/')
  ...
  if (userId !== Number(req.user.id)) {
    return res.status(403).json({ error: 'You can only initiate payment for yourself.' })
  }

  const amountNpr = roundMoney(requestedAmount)
  if (amountNpr < MIN_TOPUP_NPR) {
    return res.status(400).json({ error: `Minimum top-up is NPR ${MIN_TOPUP_NPR.toLocaleString()}.` })
  }
  if (amountNpr > MAX_TOPUP_NPR) {
    return res.status(400).json({ error: `Maximum single top-up is NPR ${MAX_TOPUP_NPR.toLocaleString()}.` })
  }
  ...
  try {
    const userRow = await pool.query('SELECT id, name, email, phone FROM users WHERE id = $1', [userId])
    ...
    if (!kh.pidx || !kh.payment_url) {
      return res.status(502).json({ error: 'Invalid response from Khalti initiate API.' })
    }
...
router.get('/khalti/callback', async (req, res) => {
  ...
  try {
    ...
    const txRes = await client.query('SELECT * FROM khalti_transactions WHERE pidx = $1 FOR UPDATE', [String(pidx)])
    if (txRes.rows.length === 0) {
      await client.query('ROLLBACK')
      return res.status(404).json({ error: 'Transaction not found for provided pidx.' })
    }
    const tx = txRes.rows[0]

    const lookup = await khaltiPost('/epayment/lookup/', { pidx: String(pidx) })
    const normalizedStatus = mapKhaltiStatus(lookup.status)

    if (normalizedStatus === 'completed' && tx.status === 'completed') {
      const balRes = await client.query('SELECT wallet_balance FROM users WHERE id = $1', [tx.user_id])
      await client.query('COMMIT')
      return res.json({
        status: 'completed',
        message: 'Payment already verified and wallet already credited.',
        credited_npr: Number(tx.amount_npr),
        total_balance_npr: balRes.rows.length ? Number(balRes.rows[0].wallet_balance) : null,
      })
    }
    ...
    if (normalizedStatus === 'pending') {
      return res.json({ status: 'pending', message: 'Payment is pending confirmation.' })
    }

    return res.json({
      status: normalizedStatus,
      message: 'Payment failed, expired, or cancelled. Wallet was not credited.',
    })
  } catch (err) {
